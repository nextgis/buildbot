...
for clean_ext in clean_exts:
    factory_deb.addStep(steps.ShellCommand(
...
# tar orginal sources
factory_deb.addStep(steps.ShellCommand(
    command=["dch.py", '-n', project_ver, '-a',
        deb_name, '-p', 'tar', '-f',
        code_dir_last],
    name="tar",
    description=["tar", "compress"],
    descriptionDone=["tar", "compressed"],
    haltOnFailure=True
))

# copy lib_cgal -> debian
factory_deb.addStep(steps.CopyDirectory(
