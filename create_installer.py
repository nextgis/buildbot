...
def repoUrl(props, platform):
    ...
    if url.startswith("https://rm.nextgis.com"):
        ...
        return "{}/{}/installer/{}/repository-{}{}".format(
            url, repo_id, repka_suffix, platform["name"], suffix_tmp
        )
...
def skip_step(step, which):
    if which == "standalone+local":
        return not (
            step.getProperty("scheduler").endswith("_standalone")
            or step.getProperty("scheduler").endswith("_local")
        )
    if which == "standalone+update":
        return not (
            step.getProperty("scheduler").endswith("_standalone")
            or step.getProperty("scheduler").endswith("_update")
        )
    if which == "create+local":
        return not (
            step.getProperty("scheduler").endswith("_create")
            or step.getProperty("scheduler").endswith("_local")
        )
...
def get_repository_http_url(props, platform):
    ...
    return get_packet_url(
        platform, repka_suffix, platform["name"] + suffix_tmp + ".zip"
    )
...
for platform in platforms:
        ...
        steps.ShellSequence(
            commands=[
                util.ShellArg(
                    command=[
                        "curl",
                        get_installer_package_url.withArgs(platform),
                        "-o",
                        "package.zip",
                        "-s",
                    ],
                ...
                util.ShellArg(
                    command=["cmake", "-E", "tar", "xzf", "package.zip"],
                    logname=logname,
                ),
            ],
    ...
    factory.addStep(
        steps.CopyDirectory(
            src=build_dir + "/qtifw_build", dest=code_dir + "/qtifw_pkg"
        )
    )
        ...
        steps.ShellSequence(
            commands=[
                util.ShellArg(
                    command=[
                        "curl",
                        get_qt_package_url.withArgs(platform),
                        "-o",
                        "package.zip",
                        "-s",
                    ],
            ...
            commands=[
                util.ShellArg(
                    command=[
                        ...
                        "-s",
                        get_repository_http_url.withArgs(platform),
                    ],
                    logname=logname,
                ),
                util.ShellArg(
    ...
    factory.addStep(
        steps.ShellCommand(
            command=[
                ...
                "versions.pkl",
                "-s",
                get_versions_url.withArgs(platform),
            ],
    ...
    factory.addStep(
        steps.ShellCommand(
            command=[
                "python3",
                "opt" + separator + "create_installer.py",
                "-s",
                "inst",
                "-q",
                "qt/bin",
                ...
                "prepare",
                "-p",
                util.Interpolate("%(prop:plugins)s"),
                "-vd",
                util.Interpolate("%(prop:valid_date)s"),
                "-vu",
            ...
            env=env_proxy,
            doStepIf=(
                lambda step: not step.getProperty("scheduler").endswith("_standalone")
            ),
        )
    )
    ...
    if "mac" == platform["name"]:
        ...
        factory.addStep(
            steps.ShellSequence(
                commands=[
                    util.ShellArg(
                        command=["pip3", "install", "--user", "dmgbuild"],
                ...
                commands=[
                    util.ShellArg(
                        command=[
                            "curl",
                            "-u",
                            "https://rm.nextgis.com/api/apple_cert/dev.p12",
                            "-o",
                            "dev.p12",
                            "-s",
                        ],
                    ...
                    util.ShellArg(
                        command=["security", "default-keychain", "-s", keychain_name],
                        logname=logname,
                    ),
                    util.ShellArg(
                        command=[
                    ...
                    util.ShellArg(
                        command=["security", "list-keychains", "-s", keychain_name],
                        logname=logname,
                    ),
                    util.ShellArg(
                        command=["security", "list-keychains"], logname=logname
                    ),
                ],
        ...
        steps.ShellCommand(
            command=[
                ...
                get_installer_name.withArgs(
                    ...
                    util.Interpolate("-%(kw:now)s", now=now),
                ),  # util.Interpolate('%(kw:basename)s%(prop:suffix)s-%(kw:now)s', basename=installer_name_base + '-standalone', now=now),
                create_opt,
                commandArgs,
            ],
    ...
    if create_updater_package:
        ...
        factory.addStep(
            steps.ShellCommand(
                ...
                haltOnFailure=True,
                doStepIf=(
                    lambda step: step.getProperty("scheduler").endswith("_create")
                ),
                workdir=build_dir,
                env=env,
            )
    ...
    factory.addStep(
        steps.ShellCommand(
            command=[
                ...
                platform["repo_id"],
                "--description",
                util.Interpolate("%(prop:notes)s"),
                "--asset_path",
                get_installer_name.withArgs(
                    build_dir_name + separator + repo_name_base, ".zip"
                ),  # util.Interpolate('%(kw:basename)s%(prop:suffix)s.zip', basename=build_dir_name + separator + repo_name_base),
                "--asset_path",
                "versions.pkl",
                "--packet_name",
                get_packet_name,
