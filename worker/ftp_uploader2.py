...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='NextGIS buildbot tools. Utility to upload json and images to ftp')
    parser.add_argument('-v', '--version', action='version', version='NextGIS buildbot version 1.0')
    parser.add_argument('--ftp_user', dest='ftp_user', required=False, help='ftp login:password')
    parser.add_argument('--ftp', dest='ftp', required=True, help='ftp url')
    parser.add_argument('--build_path', dest='build', required=True, help='path to files to upload')
