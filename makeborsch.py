...
repositories = [
    ...
    {'repo':'lib_openssl', 'args':['-DOPENSSL_NO_DYNAMIC_ENGINE=ON', '-DWITH_ZLIB=ON', '-DBUILD_APPS=ON', '-DBUILD_TESTING=ON', '-DSTATIC_RUNTIME=ON'], 'requirements':[], 'skip':['mac-static'], 'org':'nextgis-borsch', 'test_regex':[]},
    ...
    {'repo':'lib_hdf4', 'args':['-DWITH_ZLIB=ON', '-DWITH_JPEG=ON', '-DWITH_JPEG_EXTERNAL=ON', '-DWITH_SZIP=ON', '-DWITH_SZIP_EXTERNAL=ON', '-DBUILD_TESTING=ON'], 'requirements':[], 'skip':['win32-static','win64-static','mac-static'], 'org':'nextgis-borsch', 'test_regex':['-R','MFHDF_TEST-cdftest|MFHDF_TEST-clearall|MFHDF_TEST-hdfnctest|HDF_TEST-testhdf_thf']},
