...
repositories = [
    ...
    {'repo':'lib_opencv', 'args':['-DWITH_GDAL_EXTERNAL=ON','-DWITH_ZLIB_EXTERNAL=ON','-DWITH_PNG_EXTERNAL=ON','-DWITH_JPEG_EXTERNAL=ON','-DBUILD_opencv_ts=ON','-DBUILD_opencv_apps=ON','-DBUILD_TESTS=ON','-DBUILD_PERF_TESTS=OFF'], 'requirements':[], 'skip':[], 'org':'nextgis-borsch', 'test_regex':['-R','opencv_test_(fl|co)']},
]
