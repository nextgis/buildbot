...
repositories = [
    ...
    {'repo':'nextgisqgis', 'args':['-DWITH_PROJ_EXTERNAL=ON', '-DWITH_GEOS_EXTERNAL=ON', '-DWITH_GDAL_EXTERNAL=ON', '-DWITH_EXPAT_EXTERNAL=ON', '-DWITH_SpatialIndex_EXTERNAL=ON', '-DWITH_SQLite3_EXTERNAL=ON', '-DWITH_Spatialite_EXTERNAL=ON', '-DWITH_Qca_EXTERNAL=ON', '-DWITH_LIBZIP_EXTERNAL=ON', '-DWITH_QTKEYCHAIN_EXTERNAL=ON', '-DWITH_Qt5_EXTERNAL=ON', '-DWITH_EXIV_EXTERNAL=ON'], 'requirements':['', ''], 'os':['win32','win64','mac'], 'repo_root':'git://github.com', 'org':'nextgis', 'test_regex':[]},
