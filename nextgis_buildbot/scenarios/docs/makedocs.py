...
upload_script_src = (
    "https://raw.githubusercontent.com/nextgis/buildbot/master/worker/"
    + upload_script_name
)
...
for lang in langs:
    ...
    scheduler = schedulers.SingleBranchScheduler(
        name=project_name,
        change_filter=util.ChangeFilter(project=git_project_name, branch=lang),
    ...
    factory.addStep(
        steps.ShellCommand(
            command=["make", "latexpdf", 'LATEXMKOPTS="--interaction=nonstopmode"'],
            name="Generate pdf for NextGIS GeoServices",
            description=["make", "pdf for NextGIS GeoServices"],
            workdir="build/source/docs_geoserv_prem",
            warnOnFailure=True,
            env=env,
        )
    )
    ...
    factory.addStep(
        steps.ShellSequence(
            commands=[
                ...
                util.ShellArg(
                    command=[
                        ...
                        "-avz",
                        "-e",
                        "ssh -p 57936 -i /root/.ssh/www",
                        "build/json/",
                        "user@sftpgo.data.nextgis.com:{}/".format(lang),
                    ],
                    logname=logname,
                ),
            ],
            ...
            workdir="build/source/docs_data",
            warnOnFailure=True,
            env=env,
        )
    )
