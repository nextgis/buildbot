...
for ubuntu_distribution in ubuntu_distributions:
    factory_deb.addStep(steps.ShellCommand(command=['/bin/bash', 'dch.py', '-n', gdal_ver, '-a', 
                                                'gdal', '-p', 'fill', '-f', 
...
# store changelog
factory_deb.addStep(steps.ShellCommand(command=['/bin/bash', 'dch.py' '-n', gdal_ver, '-a', 'gdal', '-p', 'store', '-f', code_dir,'-o', 'changelog'], 
